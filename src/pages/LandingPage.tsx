...
import { PrologueCinematic } from '../components/landing/PrologueCinematic';
...
export function LandingPage() {
  ...
  const showDecorations = step !== 'prologue';
  ...
  return (
    <PageTransition>
      <div
        ...
        style={{
          background:
            'linear-gradient(160deg, #FFFDE7 0%, #FCE4EC 35%, #E3F2FD 65%, #F3E5F5 100%)',
        }}
      >
        {/* 무지개 상단 띠 */}
        <div
        ...
        {/* 음소거 토글 (프롤로그 중에는 숨김 — 건너뛰기 버튼이 우측 상단을 차지) */}
        {step !== 'prologue' && (
                ...
                <div>
                  <h1
                    ...
                    style={{
                      ...
                      background:
                        'linear-gradient(135deg, #FF6B6B 0%, #FFD93D 42%, #6BCB77 72%, #4D96FF 100%)',
        ...
        {/* 리더보드 단축 (프롤로그/시작 화면에서는 숨김) */}
        {(step === 'name' || step === 'difficulty') && (
          <motion.button
            whileHover={{ scale: 1.08 }}
            whileTap={{ scale: 0.95 }}
            onClick={() => navigate('/leaderboard')}
            className="absolute bottom-6 right-6 z-20 font-fredoka text-sm px-4 py-2 rounded-full transition-all"
            style={{
